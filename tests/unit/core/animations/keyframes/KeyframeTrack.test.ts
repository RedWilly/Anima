...
describe('KeyframeTrack', () => {
    ...
    describe('keyframe modification affects animation', () => {
        it('modifying keyframe value via addKeyframe changes getValueAt output', () => {
            const track = new KeyframeTrack();
            track.addKeyframe(0, 0).addKeyframe(1, 100);

            expect(track.getValueAt(0.5)).toBe(50);

            // addKeyframe replaces existing keyframe at same time
            track.addKeyframe(1, 200);
            expect(track.getValueAt(0.5)).toBe(100);
        });

        it('modifying keyframe easing via addKeyframe changes interpolation', () => {
            const track = new KeyframeTrack();
            track.addKeyframe(0, 0).addKeyframe(1, 100);

            const linearValue = track.getValueAt(0.5);

            // addKeyframe replaces existing keyframe at same time with new easing
            track.addKeyframe(1, 100, easeOutQuad);
